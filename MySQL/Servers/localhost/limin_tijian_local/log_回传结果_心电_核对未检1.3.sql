...
-- 1. LOG表没有回传报文
DROP TEMPORARY TABLE IF EXISTS t_OrderIdDRECG;
CREATE TEMPORARY TABLE t_OrderIdDRECG AS
SELECT DISTINCT
...
SELECT * FROM t_OrderIdDRECG ORDER BY 序号,分组名称;
...
WHERE dr.del_flag <> '1'
  AND dir.del_flag <> '1'
  AND dir.order_application_id IN (SELECT 申请单号 FROM t_OrderIdDRECG)
