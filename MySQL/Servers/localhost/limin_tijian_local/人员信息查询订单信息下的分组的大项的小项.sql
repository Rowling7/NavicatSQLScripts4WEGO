...
FROM
  ...
  AND go.del_flag <> '1'
  JOIN t_order_group_item_f594102095fd9263b9ee22803eb3f4e5 ogi ON ogi.group_id = og.id
  AND ogi.del_flag <> '1'
  JOIN t_order_group_item_project_f594102095fd9263b9ee22803eb3f4e5 ogip ON ogip.t_order_group_item_id = ogi.id
  AND ogip.del_flag <> '1'
WHERE
  gp.del_flag <> '1'
  AND gp.person_name = '田晓明'
  AND gp.id_card = '824299195501309857' --	AND go.order_name='互联科技2025-07'
ORDER BY
  og.name,
  gp.person_name;