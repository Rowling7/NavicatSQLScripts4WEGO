...
SELECT DISTINCT
       ...
       CAST(gp.inspection_time AS CHAR(255)) AS 导引单打印时间,
       CAST(MIN(dir.check_date) AS CHAR(255)) AS 体检时间,
       CAST(MIN(IFNULL(gp.inspection_time, dir.check_date)) AS CHAR(255)) AS 最小时间
...
WHERE dr.del_flag <> '1'
  ...
  AND CAST(gp.inspection_time AS CHAR(255)) > CAST(now() AS CHAR(10))
GROUP BY
    go.order_code,
    go.order_name,
    og.name,
