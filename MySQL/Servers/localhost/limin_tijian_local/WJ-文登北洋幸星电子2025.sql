...
WHERE -- gp.person_name LIKE '%单美娜%'
  -- AND GP.test_num = ''
  -- AND pd.report_time <'2025-09-03 08:05:23'  
   go.order_name = '文登北洋幸星电子2025'
  AND dr.group_item_name in( '腹部彩超（含肝胆胰脾双肾）','T肾功3项','T肝功12项','T-尿常规','血常规','葡萄糖测定','常规心电图自动分析','胸部正位')
  AND gp.del_flag <> 1
  -- AND dr.del_flag <> 1
  -- AND pd.del_flag <> 1
  -- AND ld.del_flag <> 1
  ...
  and (length(dr.diagnose_sum)<1 or dr.diagnose_sum is null)
