...
SELECT DISTINCT go.order_name AS 订单名称,
-- gp.id,
gp.person_name AS 姓名,
gp.patient_id AS HIS号,
gp.test_num as 体检号,
		dr.group_item_name 体检项目,
		dr.check_date 检查时间,
	case when dr.barname='妇科检查' and dr.confirm_status=0 then '妇科检查未填写'
		else null  end  R1,
case when dr.barname ='彩超乳腺+妇科' and dr.confirm_status<>2 then '彩超乳腺+妇科未做'
		when dr.barname ='液基薄层细胞制片术（TCT）' and dr.confirm_status<>2 then '液基薄层细胞制片术（TCT）未做'
		when dr.barname ='阴道分泌物检查' and dr.confirm_status<>2 then '阴道分泌物检查未做'
		else null  end  R2
...
WHERE gp.del_flag <> '1'
  ...
  AND go.del_flag <> '1'
	and gp.is_pass =2
	and gp.screening=1
 and go.order_name='三角妇女2025'
 and dr.group_item_name <>'总检报告'
